...
use mailgrit_core_domain::{BulkOperationKind, OperationTarget, RawCsvRow, SanitizedUserRow};
...
fn sample_row() -> SanitizedUserRow {
    RawCsvRow::new(vec![
    ...
    .parse()
    .expect("test row must parse")
}

// D1/D2: the verdict requires a positive marker and the extended error list.
#[test]
fn create_js_requires_success_marker_and_marks() {
...
#[test]
fn d4_verify_flag_false_disables_verification() {
    let js = build_batch_js(
        1,
        OperationTarget::User,
...
#[test]
fn d5_pii_masking_function_present() {
    let js = build_batch_js(
        1,
        OperationTarget::User,
        BulkOperationKind::Create,
...
#[test]
fn e1_error_field_uses_extract_message_not_html_slice() {
    let js = build_batch_js(
        1,
        OperationTarget::User,
...
#[test]
fn generated_js_is_balanced_iife_for_all_kinds() {
    for kind in [
        BulkOperationKind::Create,
        BulkOperationKind::Edit,
        BulkOperationKind::Delete,
